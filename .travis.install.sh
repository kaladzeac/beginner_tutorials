...
sudo apt-get install -qq -y ros-${CI_ROS_DISTRO}-desktop-full > /dev/null 2>&1
sudo rosdep init
rosdep update
...
# populate underlay
#if [ -f $TRAVIS_BUILD_DIR/.travis.rosinstall ]; then wstool init -j3 src $TRAVIS_BUILD_DIR/.travis.rosinstall; fi
#if [ ! -f $TRAVIS_BUILD_DIR/.travis.rosinstall ]; then wstool init -j3 src $DEFAULT_ROSINSTALL; fi
